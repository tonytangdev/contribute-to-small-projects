...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  let title = `Projects - Page ${page}`
  let description = `Discover more small open source projects (100-600 stars) - page ${page}`

  if (search) {
    title = `Projects matching "${search}" - Page ${page}`
    description = `Browse open source projects matching "${search}" with 100-600 stars - page ${page}`
  }

  const url = new URL(`https://www.contribute-to-small-projects.com/page/${page}`)
...
export default async function PaginatedPage({ params, searchParams }: PageProps) {
  const { pageNum } = await params
  ...
  const data = await getRepositories(currentPage, undefined, searchTerm)
  ...
  const breadcrumbItems = [
    ...
    { '@type': 'ListItem', position: 2, name: `Page ${currentPage}`, item: `${baseUrl}/page/${currentPage}` }
  ]
        ...
        <>
          ...
          <nav aria-label="Pagination" className="flex justify-center items-center gap-3 sm:gap-6 mt-12 sm:mt-16 relative">
            {pagination.hasPrevPage ? (
              <Link
                href={currentPage === 2
                  ...
                  : `/page/${pagination.currentPage - 1}${searchTerm ? `?search=${encodeURIComponent(searchTerm)}` : ''}`}
                prefetch={true}
            ...
            {pagination.hasNextPage ? (
              <Link
                href={`/page/${pagination.currentPage + 1}${searchTerm ? `?search=${encodeURIComponent(searchTerm)}` : ''}`}
                prefetch={true}
                className="group flex items-center gap-2 sm:gap-3 px-4 py-2 sm:px-8 sm:py-4 bg-white/80 backdrop-blur-sm border-2 border-slate-200 text-slate-700 rounded-xl sm:rounded-2xl hover:bg-indigo-50 hover:border-indigo-300 hover:-translate-y-0.5 transition-all duration-200 font-semibold shadow-sm text-sm sm:text-base"
              >
